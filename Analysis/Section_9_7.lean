...
namespace Chapter9
/-- Helper: the increasing branch of the intermediate value theorem (`y` between `f a` and `f b`). -/
private theorem ivt_aux {a b:ℝ} (hab: a < b) {f:ℝ → ℝ} (hf: ContinuousOn f (.Icc a b)) {y:ℝ}
    (hy_left : y ∈ Set.Icc (f a) (f b)) : ∃ c ∈ Set.Icc a b, f c = y := by
  by_cases hya : y = f a; use a; grind
  by_cases hyb : y = f b; use b; grind
  simp at hy_left
  replace hya : f a < y := by order
  replace hyb : y < f b := by order
...
theorem intermediate_value {a b:ℝ} (hab: a < b) {f:ℝ → ℝ} (hf: ContinuousOn f (.Icc a b)) {y:ℝ} (hy: y ∈ Set.Icc (f a) (f b) ∨ y ∈ Set.Icc (f b) (f a)) :
  ...
  · exact ivt_aux hab hf hy_left
  · have hy' : (-y) ∈ Set.Icc (-(f a)) (-(f b)) := by
      rw [Set.mem_Icc] at hy_right ⊢; constructor <;> linarith [hy_right.1, hy_right.2]
    obtain ⟨c, hc, hfc⟩ := ivt_aux hab hf.neg hy'
    exact ⟨c, hc, by have h : -(f c) = -y := hfc; linarith⟩
