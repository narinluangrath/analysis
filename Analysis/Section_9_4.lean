...
theorem ContinuousWithinAt.tfae (X:Set ℝ) (f: ℝ → ℝ) (x₀:ℝ) :
  ...
  tfae_have 3 → 4 := by
    intro h3 ε hε
    obtain ⟨δ, hδpos, hδx⟩ := h3 ε hε
    refine ⟨δ/2, by positivity, fun x hx hxd => ?_⟩
    have hlt : |x - x₀| < δ := lt_of_le_of_lt hxd (by linarith)
    exact le_of_lt (hδx x hx hlt)
  tfae_have 4 → 3 := by
    intro h4 ε hε
    obtain ⟨δ, hδpos, hδx⟩ := h4 (ε/2) (by linarith)
    refine ⟨δ, hδpos, fun x hx hxd => ?_⟩
    have hle : |f x - f x₀| ≤ ε/2 := hδx x hx (le_of_lt hxd)
    linarith
  tfae_finish
